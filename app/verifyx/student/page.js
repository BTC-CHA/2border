...
export default function StudentPage(){
 ...
 return <main className="vx-page"><div className="vx-wrap"><nav style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:20}}><Link className="vx-link primary" href="/verifyx/student">Assignments</Link><Link className="vx-link secondary" href="/verifyx/student/results">Results</Link><Link className="vx-link secondary" href="/verifyx/student/profile">Profile</Link></nav><header className="vx-top"><div><Link className="vx-file" href="/verifyx"><ArrowLeft size={15}/>VerifyX</Link><p className="vx-kicker" style={{marginTop:14}}>STUDENT MODE</p><h1>การบ้านของฉัน</h1><div style={{display:'grid',gap:3,marginTop:6,fontSize:13,color:'#705b50'}}><div><b>School Code :</b> {profile.institution_code}</div><div><b>ชื่อ :</b> {profile.full_name}</div><div><b>รหัสนักศึกษา :</b> {profile.student_code}</div></div></div><button className="vx-btn secondary" onClick={signOut}><LogOut size={15}/>Logout</button></header>
