...
export default function StudentPage(){
  ...
  return <main className="vx-page"><div className="vx-wrap"><header className="vx-top"><div><Link className="vx-file" href="/verifyx"><ArrowLeft size={15}/>VerifyX</Link><p className="vx-kicker" style={{marginTop:14}}>STUDENT MODE</p><h1>งานที่เปิดรับ</h1><p>{identity.fullName} · {identity.studentCode}</p></div><div className="vx-toolbar"><Link className="vx-link secondary" href="/verifyx/student/history"><History size={15}/>ประวัติการส่ง</Link><button className="vx-btn secondary" onClick={()=>{localStorage.removeItem(KEY);setIdentity(null)}}>เปลี่ยนผู้ใช้</button></div></header>{error&&<div className="vx-error">{error}</div>}<section className="vx-list">{items.length?items.map(a=><Link className="vx-item" style={{textDecoration:'none',color:'inherit'}} key={a.id} href={`/verifyx/student/assignment/${a.id}`}><div><h3>{a.title}</h3><p>{a.course||'ไม่ระบุวิชา'}</p><div className="vx-tags"><span>{a.vx_questions?.[0]?.count||0} ข้อ</span><span>{a.max_attempts} attempts</span></div></div><span className="vx-status">เปิดรับงาน</span></Link>):<div className="vx-empty">ยังไม่มี Assignment ที่เปิดรับ</div>}</section></div></main>;
}
