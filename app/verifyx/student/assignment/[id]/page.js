...
export default function StudentAssignment(){
 ...
 return <main className="vx-page"><div className="vx-wrap">
  ...
  {status==='final'?<section className="vx-card"><p className="vx-kicker">FINAL SUBMITTED</p><h2>ส่งการบ้านเรียบร้อยแล้ว</h2><p>{finalAt?`ส่งเมื่อ ${new Date(finalAt).toLocaleString('th-TH')}`:''}</p><div className="vx-success">ระบบล็อกคำตอบแล้ว และจะยังไม่แสดงคะแนนหรือผลถูก/ผิดจนกว่าครูจะปิดรับ Assignment นี้</div><div className="vx-toolbar" style={{marginTop:12}}><Link className="vx-btn secondary" href="/verifyx/student">กลับหน้าการบ้าน</Link><Link className="vx-btn secondary" href="/verifyx/student/results">ดูคะแนนที่ประกาศแล้ว</Link></div></section>:<>
  {stage==='work'&&<><div className="vx-card" style={{marginBottom:14}}><div className="vx-tags"><span>บันทึกแล้ว {completeCount}/{items.length}</span><span>{flaggedCount?`🟨 ไม่แน่ใจ ${flaggedCount} ข้อ`:'ไม่มีข้อที่ติดธง'}</span><span>{hasDirty?'มีคำตอบที่แก้แล้วแต่ยังไม่ Save':'คำตอบที่แก้ล่าสุด Save แล้ว'}</span><span>ชุดโจทย์ถูกล็อกแล้ว Refresh ก็ไม่สุ่มใหม่</span></div></div>
   <section className="vx-list">{items.map(item=>{const a=answers[item.item_id]||{};const isMcq=item.question_type==='mcq';return <article className={`vx-card ${item.is_flagged?'vx-flagged-card':''}`} key={item.item_id}>
    ...
    {isMcq?<div className="vx-form"><div className="vx-list">{(item.mcq_choices||[]).map(c=><label key={c.key} className="vx-card" style={{display:'flex',alignItems:'center',gap:12,cursor:'pointer',padding:14}}><input type="radio" name={`q-${item.item_id}`} checked={a.choice===c.key} onChange={()=>changeChoice(item.item_id,c.key)}/><b style={{minWidth:28}}>{thaiKey[c.key]||c.key}.</b><span>{c.text}</span></label>)}</div><button className="vx-btn primary" disabled={savingId!==null} onClick={()=>saveOne(item)}><Save size={15}/>{savingId===item.item_id?'กำลังบันทึก...':'บันทึกคำตอบข้อนี้'}</button></div>
    :<div className="vx-form"><div className="vx-empty" style={{marginTop:0}}>เขียนชิ้นงานตาม Drawing แล้วนำค่าจาก Mass Properties มากรอกด้านล่าง</div><div className="vx-mass-grid"><label>Volume mm³<input value={a.volume??''} onChange={e=>changeCad(item.item_id,'volume',e.target.value)} type="number" step="0.001"/></label><label>Surface Area mm²<input value={a.surface_area??''} onChange={e=>changeCad(item.item_id,'surface_area',e.target.value)} type="number" step="0.001"/></label><label>Mass g<input value={a.mass??''} onChange={e=>changeCad(item.item_id,'mass',e.target.value)} type="number" step="0.001"/></label><label>COM X<input value={a.com_x??''} onChange={e=>changeCad(item.item_id,'com_x',e.target.value)} type="number" step="0.001"/></label><label>COM Y<input value={a.com_y??''} onChange={e=>changeCad(item.item_id,'com_y',e.target.value)} type="number" step="0.001"/></label><label>COM Z<input value={a.com_z??''} onChange={e=>changeCad(item.item_id,'com_z',e.target.value)} type="number" step="0.001"/></label></div><button className="vx-btn primary" disabled={savingId!==null} onClick={()=>saveOne(item)}><Save size={15}/>{savingId===item.item_id?'กำลังบันทึก...':'บันทึกคำตอบข้อนี้'}</button></div>}
   </article>})}</section></>}
  {stage==='summary'&&<section className="vx-card"><p className="vx-kicker">SUMMARY</p><h2>ตรวจคำตอบก่อน Final</h2><p>ตรวจรายการด้านล่างให้ครบ ก่อนกดยืนยันคำตอบสุดท้าย</p><div className="vx-list">{items.map(item=>{const a=answers[item.item_id]||{};const saved=a.saved;const isMcq=item.question_type==='mcq';return <article className="vx-item" key={item.item_id}><div><h3>ข้อ {item.slot_no} · {item.title}</h3><p>{saved?(isMcq?`เลือก ${thaiKey[a.choice]||a.choice||'—'} และบันทึกแล้ว`:'กรอกค่าจาก Mass Properties ครบและบันทึกแล้ว'):'ยังไม่ได้บันทึกคำตอบครบ'}</p><div className="vx-tags">{item.is_flagged&&<span>🟨 ไม่แน่ใจ — ควรกลับไปตรวจ</span>}</div></div><button className="vx-link secondary" onClick={()=>setStage('work')}>{item.is_flagged?'ตรวจข้อนี้':'กลับไปแก้'}</button></article>})}</div><div className={canFinal?'vx-success':'vx-error'} style={{marginTop:14}}>{canFinal?(flaggedCount?`ตอบครบแล้ว แต่ยังมีธงเหลือง ${flaggedCount} ข้อ`:'ตอบครบทุกข้อแล้ว พร้อม Final'):(hasDirty?'ยังมีคำตอบที่แก้แล้วแต่ยังไม่ได้ Save':`ต้องบันทึกให้ครบ ${items.length} ข้อก่อน Final · ตอนนี้ ${completeCount}/${items.length}`)}</div><button className="vx-btn primary" style={{marginTop:12,width:'100%'}} disabled={!canFinal||finalizing} onClick={finalize}><Send size={16}/>{finalizing?'กำลังล็อกงาน...':'ยืนยันคำตอบสุดท้าย · Final'}</button></section>}
  </>}
 </div></main>;
}
