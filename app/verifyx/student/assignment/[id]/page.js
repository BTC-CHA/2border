...
const questionTypeLabel={cad:'CAD',mcq:'MULTIPLE CHOICE',short_answer:'เติมคำ'};
...
export default function StudentAssignment(){
 ...
 async function load(){
  ...
  setAnswers(Object.fromEntries(xs.map(x=>{
   ...
   if(x.question_type==='short_answer')return [x.item_id,{text:x.short_answer_text||'',saved:!!String(x.short_answer_text||'').trim(),dirty:false}];
 ...
 function changeShort(itemId,value){setAnswers(a=>({...a,[itemId]:{...(a[itemId]||{}),text:value,saved:false,dirty:true}}))}
 ...
 async function saveOne(item){
  ...
  if(item.question_type==='mcq'){
   ...
   if(!String(a.text||'').trim()){setSavingId(null);setError(`ข้อ ${item.slot_no} ยังไม่ได้กรอกคำตอบ`);return}
   ({error}=await vx.rpc('vx_student_save_short_answer_auth',{p_assignment_id:assignmentId,p_item_id:item.item_id,p_answer_text:String(a.text).trim(),p_is_flagged:item.is_flagged}));
  }else{
  ...
  setAnswers(x=>({...x,[item.item_id]:{...a,saved:true,dirty:false}}));
 ...
 async function finalize(){
  ...
  const {data,error}=await vx.rpc('vx_student_finalize_assignment_v3_auth',{p_assignment_id:assignmentId});
  setFinalizing(false);
  if(error){setError(error.message);return}
  const result=data||[];
  setItems(xs=>xs.map(x=>{const r=result.find(z=>z.item_id===x.item_id);return r?{...x,...r,progress_status:'final'}:x}));
  setStatus('final');setFinalAt(result[0]?.final_at||new Date().toISOString());setStage('summary');setMessage('ส่ง Final เรียบร้อยแล้ว ระบบล็อกคำตอบแล้ว')
 }
 ...
 const assignmentType=assignment?.assignment_type==='mixed'?'Mixed':assignment?.assignment_type==='mcq'?'Multiple Choice':assignment?.assignment_type==='short_answer'?'เติมคำ':'CAD / Engineering';
 ...
 return <main className="vx-page"><div className="vx-wrap">
  ...
  <header className="vx-top"><div><Link className="vx-file" href="/verifyx/student"><ArrowLeft size={15}/>Assignments</Link><p className="vx-kicker" style={{marginTop:14}}>ASSIGNMENT</p><h1>{assignment?.title||'Assignment'}</h1><p>{profile?.institution_name} · {assignment?.course||'ไม่ระบุวิชา'} · {assignmentType} · {items.length} ข้อ · {status==='final'?'Final แล้ว':'แก้ได้จนกว่าจะ Final'}</p></div>{status!=='final'&&<button className="vx-btn secondary" onClick={()=>setStage(stage==='work'?'summary':'work')}><ListChecks size={16}/>{stage==='work'?'Summary':'กลับไปทำโจทย์'}</button>}</header>
  ...
  {stage==='work'&&<><div className="vx-card" style={{marginBottom:14}}><div className="vx-tags"><span>บันทึกแล้ว {completeCount}/{items.length}</span><span>{flaggedCount?`🟨 ไม่แน่ใจ ${flaggedCount} ข้อ`:'ไม่มีข้อที่ติดธง'}</span><span>{hasDirty?'มีคำตอบที่แก้แล้วแต่ยังไม่ Save':'คำตอบที่แก้ล่าสุด Save แล้ว'}</span><span>ชุดโจทย์ถูกล็อกแล้ว Refresh ก็ไม่สุ่มใหม่</span></div></div>
   <section className="vx-list">{items.map(item=>{const a=answers[item.item_id]||{};const isMcq=item.question_type==='mcq';const isShort=item.question_type==='short_answer';return <article className={`vx-card ${item.is_flagged?'vx-flagged-card':''}`} key={item.item_id}>
    <div className="vx-top" style={{marginBottom:10}}><div><p className="vx-kicker">QUESTION {item.slot_no} · {questionTypeLabel[item.question_type]||item.question_type}</p><h3>{item.title}</h3></div><span className={a.saved?'vx-progress final':'vx-progress muted'}>{a.saved?'บันทึกแล้ว':'ยังไม่บันทึก'}</span></div>
    <div className="vx-toolbar">{item.question_type==='cad'&&<button className="vx-file" disabled={!item.drawing_path} onClick={()=>openFile(item.drawing_path)}><FileText size={15}/>{item.drawing_path?'Drawing PDF':'ยังไม่มี PDF'}</button>}{item.question_type==='cad'&&item.show_model_preview&&item.model_image_path&&<button className="vx-file" onClick={()=>openFile(item.model_image_path)}><Eye size={15}/>Model Reference</button>}<button className={`vx-file ${item.is_flagged?'vx-flag-active':''}`} onClick={()=>toggleFlag(item)}><Flag size={15}/>{item.is_flagged?'🟨 ไม่แน่ใจ':'มาร์กไม่แน่ใจ'}</button></div>
    ...
    {isShort&&<div className="vx-form"><label>คำตอบ<input value={a.text??''} onChange={e=>changeShort(item.item_id,e.target.value)} disabled={status==='final'} placeholder="พิมพ์คำตอบสั้น ๆ" autoComplete="off"/></label><div className="vx-empty" style={{marginTop:0}}>ระบบไม่สนตัวพิมพ์ใหญ่/เล็ก และตัดช่องว่างส่วนเกินตอนตรวจ</div>{status!=='final'&&<button className="vx-btn primary" disabled={savingId!==null} onClick={()=>saveOne(item)}><Save size={15}/>{savingId===item.item_id?'กำลังบันทึก...':'บันทึกคำตอบข้อนี้'}</button>}</div>}

    {item.question_type==='cad'&&<div className="vx-form"><div className="vx-mass-grid"><label>Volume mm³<input value={a.volume??''} onChange={e=>changeCad(item.item_id,'volume',e.target.value)} type="number" step="0.001" disabled={status==='final'}/></label><label>Surface Area mm²<input value={a.surface_area??''} onChange={e=>changeCad(item.item_id,'surface_area',e.target.value)} type="number" step="0.001" disabled={status==='final'}/></label><label>Mass g<input value={a.mass??''} onChange={e=>changeCad(item.item_id,'mass',e.target.value)} type="number" step="0.001" disabled={status==='final'}/></label><label>COM X<input value={a.com_x??''} onChange={e=>changeCad(item.item_id,'com_x',e.target.value)} type="number" step="0.001" disabled={status==='final'}/></label><label>COM Y<input value={a.com_y??''} onChange={e=>changeCad(item.item_id,'com_y',e.target.value)} type="number" step="0.001" disabled={status==='final'}/></label><label>COM Z<input value={a.com_z??''} onChange={e=>changeCad(item.item_id,'com_z',e.target.value)} type="number" step="0.001" disabled={status==='final'}/></label></div>{status!=='final'&&<button className="vx-btn primary" disabled={savingId!==null} onClick={()=>saveOne(item)}><Save size={15}/>{savingId===item.item_id?'กำลังบันทึก...':'บันทึกคำตอบข้อนี้'}</button>}</div>}
   </article>})}</section></>}

  {stage==='summary'&&<section className="vx-card"><p className="vx-kicker">SUMMARY</p><h2>{status==='final'?'ผลการตรวจ Final':'ตรวจคำตอบก่อน Final'}</h2><p>{status==='final'?`ส่ง Final แล้ว ${finalAt?new Date(finalAt).toLocaleString('th-TH'):''}`:'ตรวจรายการด้านล่างให้ครบ ก่อนกดยืนยันคำตอบสุดท้าย'}</p>
   <div className="vx-list">{items.map(item=>{const a=answers[item.item_id]||{};const saved=a.saved||status==='final';const isMcq=item.question_type==='mcq';const isShort=item.question_type==='short_answer';return <article className="vx-item" key={item.item_id}><div><h3>ข้อ {item.slot_no} · {item.title}</h3><p>{!saved?'ยังไม่ได้บันทึกคำตอบครบ':isMcq?`เลือก ${thaiKey[a.choice]||a.choice||item.selected_choice_key||'—'} และบันทึกแล้ว`:isShort?`คำตอบ: ${a.text||item.short_answer_text||'—'}`:'กรอกครบและบันทึกแล้ว'}</p><div className="vx-tags">{item.is_flagged&&status!=='final'&&<span>🟨 ไม่แน่ใจ — ควรกลับไปตรวจ</span>}{status==='final'&&isMcq&&<><span>{item.mcq_correct?'ตอบถูก':'ตอบผิด'}</span><span>Score {Number(item.score||0).toFixed(0)}/100</span></>}{status==='final'&&isShort&&<><span>{item.short_answer_correct?'ตอบถูก':'ตอบผิด'}</span><span>Score {Number(item.score||0).toFixed(0)}/100</span></>}{status==='final'&&item.question_type==='cad'&&<><span>Volume {item.volume_pass?'PASS':'FAIL'} · Error {pct(item.volume_error_percent)}</span><span>Area {item.area_pass?'PASS':'FAIL'} · Error {pct(item.area_error_percent)}</span><span>Score {Number(item.score||0).toFixed(0)}/100</span></>}</div></div>{status!=='final'&&<button className="vx-link secondary" onClick={()=>setStage('work')}>{item.is_flagged?'ตรวจข้อนี้':'กลับไปแก้'}</button>}</article>})}</div>
   {status!=='final'?<><div className={canFinal?'vx-success':'vx-error'} style={{marginTop:14}}>{canFinal?(flaggedCount?`ตอบครบแล้ว แต่ยังมีธงเหลือง ${flaggedCount} ข้อ`:'ตอบครบทุกข้อแล้ว พร้อม Final'):(hasDirty?'ยังมีคำตอบที่แก้แล้วแต่ยังไม่ได้ Save':`ต้องบันทึกให้ครบ ${items.length} ข้อก่อน Final · ตอนนี้ ${completeCount}/${items.length}`)}</div><button className="vx-btn primary" style={{marginTop:12,width:'100%'}} disabled={!canFinal||finalizing} onClick={finalize}><Send size={16}/>{finalizing?'กำลังตรวจและล็อกงาน...':'ยืนยันคำตอบสุดท้าย · Final'}</button></>:<div className="vx-result"><div className="vx-score">{items.length?Math.round(items.reduce((n,x)=>n+Number(x.score||0),0)/items.length):0}/100</div><p>คะแนนเฉลี่ยของ Assignment · ไม่เปิด Reference/เฉลยที่ระบบใช้ตรวจ</p></div>}
  </section>}
 </div></main>
}
