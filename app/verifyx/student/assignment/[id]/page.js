 ...
 return <main className="vx-page"><div className="vx-wrap vx-exam-wrap">
  ...
  {stage==='work'&&current&&<>
   ...
   <section className={`vx-card vx-exam-question ${!isMcq?'vx-cad-question':''} ${current.is_flagged?'vx-flagged-card':''}`}>
    ...
    {!isMcq&&<div className="vx-toolbar vx-exam-tools"><button className="vx-file" disabled={!current.drawing_path} onClick={()=>openFile(current.drawing_path)}><FileText size={15}/>{current.drawing_path?'Drawing PDF':'ยังไม่มี PDF'}</button></div>}
