...
export default function QuestionBankPage(){
 ...
 const [mode,setMode]=useState('cad'),[mcqTab,setMcqTab]=useState('school'),[filter,setFilter]=useState('all'),[mcqStatus,setMcqStatus]=useState('all'),[search,setSearch]=useState('');
 ...
 const shownSystem=useMemo(()=>{const q=search.toLowerCase().trim();return visibleSystemMcq.filter(x=>(filter==='all'||x.difficulty===filter)&&(!q||`${ccMcqCode(x,visibleSystemMcq)} ${x.stem} ${x.subject} ${x.category_name} ${x.category_code}`.toLowerCase().includes(q)))},[visibleSystemMcq,filter,search]);
