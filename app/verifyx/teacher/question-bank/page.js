...
export default function QuestionBankPage(){
 ...
 const shownSchool=useMemo(()=>{const q=search.toLowerCase().trim();return visibleSchoolMcq.filter(x=>(filter==='all'||x.difficulty===filter)&&(!q||`${ccMcqCode(x,visibleSchoolMcq)} ${x.stem} ${x.subject} ${x.category_name} ${x.category_code}`.toLowerCase().includes(q)))},[visibleSchoolMcq,filter,search]);
