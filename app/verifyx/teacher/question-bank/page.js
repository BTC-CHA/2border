...
import {Plus,Boxes,FileText,Image as ImageIcon,Power,Download,RefreshCw,SlidersHorizontal,Save} from 'lucide-react';
...
export default function QuestionBankPage(){
 ...
 const shownCad=useMemo(()=>{const q=search.toLowerCase().trim();return items.filter(x=>{const f=familyMap[x.family_id];return (filter==='all'||f?.difficulty===filter)&&(cadCategory==='all'||f?.category===cadCategory)&&(cadLot==='all'||(f?.lot||'General')===cadLot)&&(!q||`${x.title} ${f?.name||''} ${f?.code||''} ${f?.category||''} ${f?.lot||''}`.toLowerCase().includes(q))})},[items,familyMap,filter,search,cadCategory,cadLot]);
 ...
 async function addFamily(e){
  ...
  setSaving(false);if(error)return setError(error.message);form.reset();setShowFamily(false);setMessage('เพิ่ม Family แล้ว');load()
 }
 async function updateFamily(e,familyId){
  e.preventDefault();if(saving)return;setSaving(true);setError('');
  const fd=new FormData(e.currentTarget);
  const {error}=await vx.from('vx_question_families').update({
   category:String(fd.get('category')||'part_modeling'),
   lot:String(fd.get('lot')||'General').trim()||'General',
   updated_at:new Date().toISOString()
  }).eq('id',familyId);
  setSaving(false);if(error)return setError(error.message);setMessage('อัปเดต Category / Lot แล้ว');load()
 }
 ...
 async function addVariant(e){e.preventDefault();if(saving)return;setSaving(true);setError('');const form=e.currentTarget;try{const fd=new FormData(form),drawing=fd.get('drawing'),image=fd.get('modelImage');const drawingPath=await upload(drawing,'drawing'),imagePath=image?.name?await upload(image,'model'):null;const {error}=await vx.from('vx_question_bank').insert({family_id:Number(fd.get('familyId')),variant_code:String(fd.get('variantCode')).trim().toUpperCase(),title:String(fd.get('title')).trim(),drawing_path:drawingPath,drawing_name:drawing?.name||null,model_image_path:imagePath,model_image_name:image?.name||null,show_model_preview:Boolean(imagePath)&&fd.get('preview')==='on',reference_volume:Number(fd.get('volume')).toFixed(3),reference_area:Number(fd.get('area')).toFixed(3),reference_mass:Number(fd.get('mass')).toFixed(3),reference_com_x:0,reference_com_y:0,reference_com_z:0});if(error)throw error;form.reset();setShowVariant(false);setMessage('เพิ่ม CAD Variant แล้ว');await load()}catch(err){setError(err.message)}finally{setSaving(false)}}
 ...
 async function toggleCad(q){const value=!q.is_active;const {error}=await vx.from('vx_question_bank').update({is_active:value,updated_at:new Date().toISOString()}).eq('id',q.id);if(error)return setError(error.message);setMessage(value?'เปิดใช้ CAD Variant แล้ว':'ปิดใช้ CAD Variant แล้ว');load()}
 ...
 return <main className="vx-page"><div className="vx-wrap"><TeacherNav active="bank"/>
  <header className="vx-top"><div><p className="vx-kicker">QUESTION BANK</p><h1>คลังโจทย์</h1><p>School Central Question Bank · CAD Family/Variant + ปรนัย พร้อม Category และ Lot</p></div><div className="vx-toolbar"><button className="vx-btn secondary" onClick={load}><RefreshCw size={15}/>Refresh</button>{mode==='cad'?<><button className="vx-btn secondary" onClick={()=>setShowOrganize(v=>!v)}><SlidersHorizontal size={15}/>จัดหมวด Family</button><button className="vx-btn secondary" onClick={()=>setShowFamily(v=>!v)}><Boxes size={15}/>เพิ่ม Family</button><button className="vx-btn primary" onClick={()=>setShowVariant(v=>!v)}><Plus size={15}/>เพิ่ม Variant</button></>:mcqTab==='school'?<button className="vx-btn primary" onClick={()=>setShowMcq(v=>!v)}><Plus size={15}/>เพิ่มข้อปรนัย</button>:<button className="vx-btn primary" disabled={!selectedSystem.length||saving} onClick={importSelected}><Download size={15}/>นำเข้า {selectedSystem.length} ข้อ</button>}</div></header>
  {message&&<div className="vx-success">{message}</div>}{error&&<div className="vx-error">{error}</div>}

  <div className="vx-toolbar" style={{marginTop:14}}><button className={`vx-btn ${mode==='cad'?'primary':'secondary'}`} onClick={()=>setMode('cad')}>CAD / Mass Properties</button><button className={`vx-btn ${mode==='mcq'?'primary':'secondary'}`} onClick={()=>{setMode('mcq');setShowFamily(false);setShowVariant(false);setShowOrganize(false)}}>ปรนัย / Multiple Choice</button></div>
  ...
  <section className="vx-card" style={{marginTop:10,padding:12}}>
   <div className="vx-form-row"><label>ค้นหา<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="คำถาม / Family / Category / Lot"/></label><label>Difficulty<select value={filter} onChange={e=>setFilter(e.target.value)}><option value="all">ทั้งหมด</option><option value="basic">Basic</option><option value="pro">Pro</option><option value="advanced">Advanced</option></select></label></div>
  ...
  {mode==='cad'&&showOrganize&&<section className="vx-card"><p className="vx-kicker">ORGANIZE</p><h3>จัด Category / Lot ของ Family</h3><p>Variant ทุกตัวใน Family เดียวกันจะใช้ Category และ Lot เดียวกัน</p><div className="vx-list">{families.map(f=><form key={f.id} className="vx-item" onSubmit={e=>updateFamily(e,f.id)} style={{alignItems:'end'}}><div style={{flex:'1 1 220px'}}><b>{f.code}</b><p>{diffLabel[f.difficulty]} · {categoryLabel[f.category]||f.category} · Lot {f.lot||'General'}</p></div><label style={{minWidth:180}}>Category<select name="category" defaultValue={f.category||'part_modeling'}>{categoryOptions.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label><label style={{minWidth:150}}>Lot<input name="lot" defaultValue={f.lot||'General'} placeholder="เช่น LOT-01"/></label><button className="vx-file" disabled={saving}><Save size={14}/>บันทึก</button></form>)}</div></section>}

  {mode==='cad'&&showFamily&&<section className="vx-card"><h3>เพิ่ม Question Family</h3><form className="vx-form" onSubmit={addFamily}><div className="vx-form-row"><label>Family Code<input name="code" placeholder="P-001" required/></label><label>ระดับ<select name="difficulty" defaultValue="basic"><option value="basic">Basic</option><option value="pro">Pro</option><option value="advanced">Advanced</option></select></label></div><div className="vx-form-row"><label>Category<select name="category" defaultValue="part_modeling">{categoryOptions.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label><label>Lot<div style={{display:'grid',gridTemplateColumns:'minmax(100px,1fr) auto minmax(86px,110px)',gap:8,alignItems:'center'}}><input value={teacherCode||'กำลังโหลด...'} readOnly aria-label="Teacher Lot Prefix"/><b>-</b><input name="lotCode" defaultValue="001" inputMode="numeric" pattern="[0-9]{3}" maxLength={3} placeholder="001" required/></div></label></div><label>คำอธิบาย<textarea name="description" rows="2"/></label><button className="vx-btn primary" disabled={saving||!teacherCode}>{saving?'กำลังบันทึก...':'บันทึก Family'}</button></form></section>}

  {mode==='cad'&&showVariant&&<section className="vx-card"><h3>เพิ่ม CAD Variant</h3><form className="vx-form" onSubmit={addVariant}><div className="vx-form-row"><label>Family<select name="familyId" required defaultValue=""><option value="" disabled>เลือก Family</option>{families.map(f=><option key={f.id} value={f.id}>{f.code} · {categoryLabel[f.category]||f.category} · {f.lot||'General'} · {diffLabel[f.difficulty]}</option>)}</select></label><label>Variant Code<input name="variantCode" placeholder="A" required/></label></div><label>ชื่อโจทย์<input name="title" required/></label><label>Drawing PDF<input name="drawing" type="file" accept="application/pdf,.pdf" required/></label><div className="vx-mass-grid"><label>Volume mm³<input name="volume" type="number" step="0.001" required/></label><label>Surface Area mm²<input name="area" type="number" step="0.001" required/></label><label>Mass g<input name="mass" type="number" step="0.001" required/></label></div><label>Model Reference Image<input name="modelImage" type="file" accept="image/png,image/jpeg,image/webp"/></label><label><span><input name="preview" type="checkbox"/> แสดง Model Image ให้นักเรียนเห็น</span></label><button className="vx-btn primary" disabled={saving}>{saving?'กำลังบันทึก...':'บันทึก Variant'}</button></form></section>}
