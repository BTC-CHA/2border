...
function QuestionSetting({q,onReload}){
 const [v,setV]=useState(Number(q.volume_tolerance_percent||1)); const [a,setA]=useState(Number(q.area_tolerance_percent||1));
...
function AssignmentSetting({item,onReload}){
 const [status,setStatus]=useState(item.status); async function save(){const {error}=await vx.from('vx_assignments').update({status,max_attempts:1,updated_at:new Date().toISOString()}).eq('id',item.id);if(error)return alert(error.message);onReload()}
...
export default function SettingsPage(){
 const [items,setItems]=useState([]),[institution,setInstitution]=useState(null),[loading,setLoading]=useState(true),[error,setError]=useState('');
 async function load(){setLoading(true);setError('');const [{data,error},{data:inst,error:ie}]=await Promise.all([vx.from('vx_assignments').select('*, vx_questions(*)').order('created_at',{ascending:false}),vx.rpc('vx_teacher_institution')]);if(error||ie)setError((error||ie).message);setItems(data||[]);setInstitution(inst?.[0]||null);setLoading(false)}
 useEffect(()=>{load()},[]);
 return <main className="vx-page"><div className="vx-wrap"><TeacherNav active="settings"/><div className="vx-top"><div><p className="vx-kicker">TEACHER MODE</p><h1>Settings</h1><p>ตั้งค่าโรงเรียน Assignment และ Tolerance</p></div></div>{error&&<p style={{color:'#b85d58'}}>{error}</p>}{institution&&<section className="vx-card" style={{marginBottom:16}}><p className="vx-kicker">INSTITUTION</p><h2>{institution.name}</h2><p>School Code สำหรับให้นักเรียนกรอกตอนเข้า Student Mode</p><div className="vx-result"><small>School Code</small><div className="vx-score" style={{fontSize:24}}>{institution.code}</div></div><p style={{marginBottom:0}}>ข้อมูลของ Teacher, Students, Assignments, Question Bank และ Results ถูกแยกตามโรงเรียนนี้</p></section>}{loading?<p>กำลังโหลด...</p>:items.map(i=><AssignmentSetting key={i.id} item={i} onReload={load}/>)}</div></main>;
}
