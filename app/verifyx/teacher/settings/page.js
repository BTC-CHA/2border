...
function AssignmentSetting({item,onReload}){
 const [status,setStatus]=useState(item.status),[attempts,setAttempts]=useState(item.max_attempts);
 async function save(){const {error}=await vx.from('vx_assignments').update({status,max_attempts:Number(attempts),updated_at:new Date().toISOString()}).eq('id',item.id);if(error)return alert(error.message);onReload()}
 return <article className="vx-card" style={{marginBottom:12}}><h2 style={{marginTop:0}}>{item.title}</h2><p style={{color:'#927667'}}>{item.course||'ไม่ระบุวิชา'}</p><div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'end',marginBottom:12}}><label>สถานะ<select value={status} onChange={e=>setStatus(e.target.value)} style={{display:'block',padding:9,borderRadius:10,border:'1px solid #ead6ca'}}><option value="draft">ฉบับร่าง</option><option value="open">เปิดรับงาน</option><option value="closed">ปิดรับงาน</option></select></label><label>Attempts / ข้อ<input type="number" min="1" value={attempts} onChange={e=>setAttempts(e.target.value)} style={{display:'block',width:110,padding:9,borderRadius:10,border:'1px solid #ead6ca'}}/></label><button className="vx-link primary" onClick={save}>บันทึก Assignment</button></div><div style={{display:'grid',gap:8}}>{(item.vx_questions||[]).sort((x,y)=>x.question_number-y.question_number).map(q=><QuestionSetting key={q.id} q={q} onReload={onReload}/>)}</div></article>
}
...
export default function SettingsPage(){
 ...
 useEffect(()=>{load()},[]);
 return <main className="vx-page"><div className="vx-wrap"><TeacherNav active="settings"/><div className="vx-top"><div><p className="vx-kicker">TEACHER MODE</p><h1>Settings</h1><p>ตั้งค่าสถานะ Attempts และ Tolerance</p></div></div>{error&&<p style={{color:'#b85d58'}}>{error}</p>}{loading?<p>กำลังโหลด...</p>:items.map(i=><AssignmentSetting key={i.id} item={i} onReload={load}/>)}</div></main>;
}
