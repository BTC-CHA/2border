...
export default function TeacherPage(){
  ...
  async function create(e){
    ...
    try{
      const {error}=await vx.from('vx_assignments').insert({title:String(fd.get('title')||'').trim(),course:String(fd.get('course')||'').trim(),description:String(fd.get('description')||'').trim(),status:fd.get('status')||'draft',max_attempts:Number(fd.get('maxAttempts')||3),question_category:'part_modeling',difficulty:String(fd.get('difficulty')||'basic'),question_count:Number(fd.get('questionCount')||1),randomize_questions:fd.get('randomize')==='on'});
  ...
  return <main className="vx-page"><div className="vx-wrap">
    ...
    {show&&<section className="vx-card"><h3>สร้าง Assignment</h3><form className="vx-form" onSubmit={create}><label>ชื่อ Assignment<input name="title" required/></label><label>วิชา / Course<input name="course"/></label><label>คำอธิบาย<textarea name="description" rows="3"/></label><div className="vx-form-row"><label>หมวด<select name="category" defaultValue="part_modeling" disabled><option value="part_modeling">Part Modeling</option></select></label><label>ระดับ<select name="difficulty" defaultValue="basic"><option value="basic">Basic</option><option value="pro">Pro</option><option value="advanced">Advanced</option></select></label></div><div className="vx-form-row"><label>จำนวนข้อ<input name="questionCount" type="number" min="1" defaultValue="3" required/></label><label>Attempts / ข้อ<input name="maxAttempts" type="number" min="1" defaultValue="3"/></label></div><div className="vx-form-row"><label>สถานะ<select name="status" defaultValue="draft"><option value="draft">ฉบับร่าง</option><option value="open">เปิดรับงาน</option><option value="closed">ปิดรับงาน</option></select></label><label><span><input name="randomize" type="checkbox" defaultChecked/> สุ่มโจทย์ให้นักเรียน</span></label></div><div className="vx-empty" style={{marginTop:0}}>ระบบจะสุ่มจาก Family คนละชุด และจะไม่ให้ Family เดียวกันซ้ำในชุดนักเรียนเดียว</div><button className="vx-btn primary" disabled={saving}>{saving?'กำลังสร้าง...':'บันทึก'}</button></form></section>}
    {error&&<div className="vx-error">{error}</div>}
    {loading?<div className="vx-empty">กำลังโหลด...</div>:<section className="vx-list">{items.length?items.map(a=>{const oldCount=a.vx_questions?.[0]?.count||0;return <article className="vx-item" key={a.id}><div><h3>{a.title}</h3><p>{a.course||'ไม่ระบุวิชา'}</p><div className="vx-tags"><span>Part Modeling</span><span>{diffLabel[a.difficulty]||a.difficulty}</span><span>{a.randomize_questions?'สุ่ม':'ไม่สุ่ม'} {a.question_count} ข้อ</span><span>{a.max_attempts} attempts</span>{oldCount>0&&<span>โจทย์เดิม {oldCount} ข้อ</span>}</div></div><div className="vx-toolbar"><span className="vx-status">{statusLabel[a.status]||a.status}</span>{a.status!=='open'&&<button className="vx-link secondary" disabled={busyId===a.id} onClick={()=>setStatus(a,'open')}>เปิดรับงาน</button>}{a.status==='open'&&<button className="vx-link secondary" disabled={busyId===a.id} onClick={()=>setStatus(a,'closed')}>ปิดรับงาน</button>}<Link className="vx-link secondary" href={`/verifyx/teacher/assignment/${a.id}`}>จัดการ</Link>{oldCount===0&&<button className="vx-link secondary" disabled={busyId===a.id} onClick={()=>removeEmpty(a)}><Trash2 size={14}/>ลบ</button>}</div></article>}):<div className="vx-empty">ยังไม่มี Assignment</div>}</section>}
  </div></main>
}
