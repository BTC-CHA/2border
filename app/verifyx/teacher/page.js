...
export default function TeacherPage(){
  ...
  async function create(e){
    ...
    try{
      const {error}=await vx.from('vx_assignments').insert({title:String(fd.get('title')||'').trim(),course:String(fd.get('course')||'').trim(),description:String(fd.get('description')||'').trim(),status:fd.get('status')||'draft',max_attempts:1,question_category:'part_modeling',difficulty:String(fd.get('difficulty')||'basic'),question_count:Number(fd.get('questionCount')||1),randomize_questions:fd.get('randomize')==='on'});
